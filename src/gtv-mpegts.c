...
static void gtv_mpegts_pat ( GstMpegtsSection *section )
{
    GPtrArray *pat = gst_mpegts_section_get_pat ( section );
    g_debug ( "PAT: %d Programs \n", pat->len );
...
static void gtv_mpegts_pmt ( GstMpegtsSection *section )
{
    ...
    g_debug ( "PMT: %d  ( %d ) \n", gtvmpegts.pmt_count+1, len );

    g_debug ( "     Program number     : %d (0x%04x) \n", pmt->program_number, pmt->program_number );
    g_debug ( "     Pcr pid            : %d (0x%04x) \n", pmt->pcr_pid, pmt->pcr_pid );
    g_debug ( "     %d Streams: \n", len );
    ...
    for ( i = 0; i < len; i++ )
    {
        ...
        g_debug ( "       pid: %d (0x%04x), stream_type:0x%02x (%s) \n", stream->pid, stream->pid, stream->stream_type,
...
static void gtv_mpegts_sdt ( GstMpegtsSection *section )
{
    ...
    g_debug ( "Services: %d  ( %d ) \n", gtvmpegts.sdt_count+1, len );
    ...
    for ( i = 0; i < len; i++ )
    {
        ...
        g_debug ( "     Service id: %d  %d 0x%04x \n", gtvmpegts.sdt_count+1, service->service_id, service->service_id );
...
static void gtv_mpegts_vct ( GstMpegtsSection *section )
{
    ...
    for ( i = 0; i < vct->sources->len; i++ )
    {
        ...
        g_debug ( "     Service id: %d  %d 0x%04x \n", gtvmpegts.vct_count+1, source->program_number, source->program_number );
        g_debug ( "     Service  (name) : %s \n", source->short_name );
...
void gtv_mpegts_stop ()
{
    ...
    if ( ( gtvmpegts.t_cur - gtvmpegts.t_start ) >= 10 )
    {
        g_debug ( "Warning. Time stop %ld (sec) \n", gtvmpegts.t_cur - gtvmpegts.t_start );
        gtv_scan_stop ( NULL, NULL );
    }
}
...
static void gtv_mpegts_pmt_lang ( GstMpegtsSection *section )
{
    const GstMpegtsPMT *pmt = gst_mpegts_section_get_pmt ( section );
    GPtrArray *streams = pmt->streams;
    guint num = 0;

    if ( gtvdvb.sid == pmt->program_number )
    {
        for ( j = 0; j < streams->len; j++ )
        {
            GstMpegtsPMTStream *pmtstream = g_ptr_array_index ( streams, j );
            const gchar *name_t = dvb_mpegts_enum_name ( GST_TYPE_MPEGTS_STREAM_TYPE, pmtstream->stream_type );
            ...
            if ( g_strrstr ( name_t, "audio" ) )
            {
                ...
                gtv_set_combo_lang ( lang, pmtstream->pid );
                                
                g_debug ( "       lang: %s | %d | num %d \n", lang ? lang : "none", pmtstream->pid, num );
