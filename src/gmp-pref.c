...
{
	g_debug ( "gmp_pref_get_time_date_str \n" );
...
void gmp_add_filter ( GtkFileChooserDialog *dialog, gchar *name, gchar *filter )
{
...
gchar * gmp_pref_open_file ( const gchar *path )
{
    ...
    gmp_add_filter ( dialog, "conf", "*.conf" );
...
void gmp_pref_save_config ( const gchar *file )
{
    ...
    GString *gstring = g_string_new ( "# Gmp-Dvb conf \n" );
